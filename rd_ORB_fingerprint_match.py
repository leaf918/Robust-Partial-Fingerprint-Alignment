...
def estimate_transform_in2fingerprint(img0, img1, patch_edge_half, ax):
    ...
    # ax[1].axis('off')
    # ax[1].set_title("match full _ patial")
    # plt.show()
...
def run(path):
    try:
        ...
        edge = int(edge[edge.find('edge') + 4:])/2
...
# def run_one():
#     try:
#         path = glob('dataset_train_liqiang_20220701_071347_num8000_edge40/*')[0]
#         # edge = os.path.split(path)[0]
#         edge = 40
#         pimg = np.load(path, allow_pickle=True)
