...
for fp in tqdm.tqdm(glob.glob('*.log')):
    ...
    for l in lns:
        ...
        if 'start training' in l:
            ...
            cur['end'] = l
    trains_ins.append(cur)
print("trains len %s" % len(trains_ins))
# 过滤有结尾的日志，去除训练中断的
full_trains = [i for i in trains_ins if len(i) == 4]
...
for train in full_trains:
    ...
    stat_list.append({
        'arch': cur_model_name,
        'corner': cur_corner_pattern,
        'best_eval': cur_min_loss_eval,
        'best_train': cur_min_loss_train
    })
    ...
    # plt.show()
pd.DataFrame(stat_list).to_csv("stat_%s.csv" % ds)
